...
if __name__ == "__main__":
    ...
    reward=read_large_csv('/local/scratch/exported/Ethereum_token_txs_data_TY_23/rewards/eth2_reward_ether/daily_attestation_reward/total_daily_attestation_rewards_sum__stakingpool_level.csv')
    ...
    reward1=reward[['date','total_reward']]
    ...
    for i in tqdm(range(len(indexs))):  
        ...
        for j in tqdm(reward1.columns):
            ...
            if indexs[i] in [index,gini,HHI]:
                ...
                if j in ['total_reward','Attestation reward']:
            ...
            IndexValues.to_csv(f'/local/scratch/exported/Ethereum_token_txs_data_TY_23/rewards/mining_staking/staking/decentralization_index/{index_name[i]}_{j}_pos_stakingpool_level.csv',index=False)
